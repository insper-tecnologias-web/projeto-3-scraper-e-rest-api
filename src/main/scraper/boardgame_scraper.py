...
class BoardgameScraper(Scraper):
    ...
    def _attribute_number_of_players_to_data(self):
        players_span_range = self._get_players_span_range()
        try:
    ...
    def _get_game_duration_span_range(self):
        li_tag = self._get_li_tags()[1]
    ...
    def _attribute_game_duration_to_data(self):
        game_duration_span_range = self._get_game_duration_span_range()
        try:
    ...
    def _attribute_description_to_data(self):
        soup = self._soup_response()
    ...
    def _handle_boardgame_data(self):
        self._attribute_number_of_players_to_data()
        self._attribute_game_duration_to_data()
        self._attribute_description_to_data()
